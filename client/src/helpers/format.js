...
export const GetFormatedCurrency = (props) => {
    const { value = 0, isFixed = 1 } = props;
    const currency = useSelector((state) => state.account.selectedCurrency, []) || '';
    const amount = isFixed == 1 ? currency + parseFloat(value).toFixed(2) : currency + parseFloat(value);
...
export const GetFormatedPnl = (props) => {
    ...
    const conVal = Number(parseFloat(value).toFixed(2));
